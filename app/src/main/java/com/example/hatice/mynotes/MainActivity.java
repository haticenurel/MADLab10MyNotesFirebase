...
public class MainActivity extends AppCompatActivity implements NoteFragment.OnNoteListInteractionListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!displayingEditor){
            ...
            ft.replace(R.id.container,NoteFragment.newInstance(),"list_note");
            ft.commit();
        }else{
            FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
    ...
    public void onBackPressed() {
        ...
        } super.onBackPressed(); if (content !=null) {
    ...
    private void saveContent(Note editingNote, String content) {
        if (editingNote.getContent() == null || !editingNote.getContent().equals(content)) {
            FirebaseFirestore db = FirebaseFirestore.getInstance();
            editingNote.setDate(new Timestamp(new Date())); editingNote.setContent(content);
            db.collection("notes").document(editingNote.getId()).set(editingNote);
        }else{
            Log.d(TAG, "notes: " + notes); NoteFragment listFragment = (NoteFragment)
                    getSupportFragmentManager().findFragmentByTag("list_note"); listFragment.updateNotes(notes);
        }
    }
    ...
    @Override
    public void onNoteSelected(Note item) {
